...
async function openrouterIA(historicoGlobal, api, model) {
    const modelUsed = model;

    const dataAtual = new Date().toLocaleString('sv-SE', { timeZone: 'America/Sao_Paulo' }).replace('T', ' ').split('.')[0];
    
    // Adiciona o prompt de sistema
    let mensagens = [
        {
            role: "system",
            content: `Você é um assistente de gestão de delivery com total autonomia para gerenciar dados (CRUD). Seu código secreto é o OPEN ROUTER. Você pode gerar dados aleatórios realistas para testes quando solicitado. 
    ...
    try {
        while (true) {
            ...
            for (const toolCall of mensagemIA.tool_calls) {
                ...
                console.log(`📡 Executando OpenRouter Tool: ${toolCall.function.name}`, args);
        ...
        console.error("Erro Agente OpenRouter:", error);
        return `⚠️ Erro no OpenRouter: ${error.message}`;
    }
}

export default openrouterIA;
