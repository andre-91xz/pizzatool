...
// Cache em memória para relatórios financeiros
const cacheRelatorios = new Map();
const cacheEstoqueValorado = new Map();
...
app.get("/api/estoque/valorado", async (req, res) => {
  ...
  const cacheKey = `estoque_valorado_${paginaNum}_${limiteNum}`;
  const cached = cacheEstoqueValorado.get(cacheKey);
  ...
  try {
    ...
    const [totalMateriaisCount, totalBebidasCount] = await Promise.all([
      pool.query('SELECT COUNT(*) FROM material WHERE status = true'),
      pool.query('SELECT COUNT(*) FROM bebidas WHERE status = true'),
    ]);
    const totalMat = parseInt(totalMateriaisCount.rows[0]?.count || 0);
    const totalBeb = parseInt(totalBebidasCount.rows[0]?.count || 0);

    res.json({
      pagina: paginaNum,
      limite: limiteNum,
      total_materiais: totalMat,
      total_bebidas: totalBeb,
      total_paginas_materiais: Math.ceil(totalMat / limiteNum),
      total_paginas_bebidas: Math.ceil(totalBeb / limiteNum),
    ...
    res.json(resposta);
    cacheEstoqueValorado.set(cacheKey, { data: resposta, timestamp: Date.now() });
  } catch (err) {
...
app.get("/api/financeiro/relatorios", async (req, res) => {
  ...
  const cacheKey = `relatorios_${periodo}`;
  const cached = cacheRelatorios.get(cacheKey);
  if (cached && (Date.now() - cached.timestamp < CACHE_TTL_MS)) {
    return res.json(cached.data);
  }
  ...
  try {
    ...
    cacheRelatorios.set(cacheKey, { data: dadosResposta, timestamp: Date.now() });
